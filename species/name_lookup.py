...
def name_lookup(q=None, rank=None, higherTaxonKey=None, status=None,
                isExtinct=None,
                habitat=None, nameType=None, datasetKey=None,
                nomenclaturalStatus=None,
                limit=100, offset=None, facet=False, facetMincount=None,
    ...
    args = {'q': q, 'rank': rank, 'higherTaxonKey': higherTaxonKey,
            ...
            'nomenclaturalStatus': nomenclaturalStatus, 'limit': limit,
            'offset': offset,
            'facet': bn(facet), 'facetMincount': facetMincount,
            'facetMultiselect': facetMultiselect,
            'hl': bn(hl), 'verbose': bn(verbose), 'type': type}
    gbif_kwargs = {key: kwargs[key] for key in kwargs if
                   key not in requests_argset}
    if gbif_kwargs is not None:
        xx = dict(zip([re.sub('_', '.', x) for x in gbif_kwargs.keys()],
                      gbif_kwargs.values()))
        args.update(xx)
